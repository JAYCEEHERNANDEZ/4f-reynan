...
import { Phone, Mail, MapPin, Send, ChevronRight, Sparkles, ArrowRight, Star, Clock, Shield, Search, Package, Loader2, CheckCircle2, Circle } from 'lucide-react'
...
export default function LandingPage() {
  ...
  const TRACK_STAGES = ['pending','washing','drying','folding','ready','released']
  const STAGE_LABELS = {
    pending: 'Pending', washing: 'Wash',
    drying: 'Dry', folding: 'Fold',
    ready: 'Ready', released: 'Released'
  }
  ...
  const handleTrack = async (e) => {
    ...
    try {
      ...
      if (enriched.length === 0) setTrackError('No orders found for this order number')
    } catch (err) {
    ...
    <div className="landing-page">
      ...
      <nav className={`landing-nav ${scrolled ? 'landing-nav-scrolled' : ''}`}>
        <div className="landing-container landing-nav-inner">
          <div className="landing-logo">
            <img src="/assets/Rectangle.png" alt="4J Laundry" className="landing-logo-img-nav" />
            <span>4J Laundry</span>
          </div>
          <div className="landing-nav-links">
            <a href="#home" onClick={(e) => scrollToSection(e, 'home')}>Home</a>
            <a href="#process" onClick={(e) => scrollToSection(e, 'process')}>Process</a>
            <a href="#track" onClick={(e) => scrollToSection(e, 'track')}>Track</a>
      ...
      <section className="landing-hero" id="home">
        ...
        <div className="landing-container landing-hero-inner" ref={heroRef}>
          <div className={`landing-hero-text ${heroVisible ? 'animate-in' : ''}`}>
            <div className="landing-hero-badge">
              <Sparkles size={14} />
              <span>Professional Laundry Service</span>
            </div>
            <h1>
              Simplify Your Life with <span className="text-highlight">4J Laundry</span> Service
            </h1>
            <p>
              With over 3 years of experience, 4J Laundry has been dedicated to providing exceptional laundry services to our community. We take pride in delivering fresh, clean, and perfectly folded garments every time.
            </p>
            <div className="landing-hero-buttons">
              <button className="landing-btn-primary landing-btn-glow" onClick={() => navigate('/login')}>
                Login
                <ChevronRight size={18} />
              </button>
              <button className="landing-btn-secondary" onClick={(e) => scrollToSection(e, 'track')}>
                Track Garment
              </button>
            </div>
          ...
          <div className={`landing-hero-image ${heroVisible ? 'animate-in-right' : ''}`}>
      ...
      <section className="landing-process" id="process" ref={processRef}>
        <div className="landing-container">
          <div className={`landing-section-header ${processVisible ? 'animate-in' : ''}`}>
            <span className="landing-section-tag">How It Works</span>
            <h2>Our Working Process</h2>
            <p>Simple steps to get your clothes fresh and clean</p>
          </div>
          <div className={`landing-process-steps ${processVisible ? 'steps-animate' : ''}`}>
            {[
              { img: '/assets/image%2011.png', alt: 'Walk-in', num: '01', title: 'Walk-in to Store', desc: 'Visit our store at Brgy. Palikpikan, Balayan, Batangas' },
              null,
              { img: '/assets/image%2014.png', alt: 'Collection', num: '02', title: 'Laundry Collection', desc: 'We carefully collect and sort your garments' },
              null,
              { img: '/assets/image%2013.png', alt: 'Cleaning', num: '03', title: 'Expert Cleaning', desc: 'Professional washing, drying, and folding' },
              null,
              { img: '/assets/image%2012.png', alt: 'Pickup', num: '04', title: 'Ready for Pickup', desc: 'Pick up your fresh, clean clothes anytime' },
            ].map((item, i) =>
              item === null ? (
                ...
                <div className="landing-step" key={item.num} style={{ animationDelay: `${parseInt(item.num) * 0.15}s` }}>
      ...
      <section className="landing-track" id="track" ref={trackRef}>
        <div className="landing-container">
          <div className={`landing-section-header ${trackVisible ? 'animate-in' : ''}`}>
          ...
          <div className={`track-search-box ${trackVisible ? 'animate-in' : ''}`}>
            <form className="track-form" onSubmit={handleTrack}>
              ...
              <button type="submit" className="landing-btn-primary landing-btn-glow" disabled={trackLoading}>
                {trackLoading ? <Loader2 size={18} className="spin" /> : <Search size={18} />}
                  ...
                  <div className="track-card" key={order.id}>
                    <div className="track-card-header">
                      <div className="track-card-info">
                        <span className="track-order-num">#{order.order_number}</span>
                        <span className="track-customer">{order.customer_name}</span>
                      </div>
                      <div className="track-card-meta">
                        <span className="track-service">{order.service_name}</span>
                    ...
                    <div className="track-progress">
                      {TRACK_STAGES.map((stage, i) => {
                        ...
                        return (
                          <div className={`track-step ${done ? 'done' : ''} ${isCurrent ? 'current' : ''}`} key={stage}>
                            <div className="track-dot">
                              {done ? <CheckCircle2 size={16} /> : <Circle size={16} />}
                            </div>
                            {i < TRACK_STAGES.length - 1 && <div className={`track-line ${done && i < currentIdx ? 'filled' : ''}`} />}
                            <span className="track-label">{STAGE_LABELS[stage]}</span>
                          </div>
                        )
                      })}
                    </div>
                    <div className="track-card-footer">
                      <span>Status: <strong className={`status-text status-${order.status}`}>{STAGE_LABELS[order.status] || order.status}</strong></span>
                      <span>Placed: {new Date(order.created_at).toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                    </div>
                  </div>
                )
              })}
      ...
      <section className="landing-contact" id="contact" ref={contactRef}>
        <div className="landing-container">
          <div className={`landing-section-header ${contactVisible ? 'animate-in' : ''}`}>
            <span className="landing-section-tag">Get In Touch</span>
            <h2>Let's Talk With Us</h2>
            <p>Have questions? We'd love to hear from you.</p>
          </div>
          <div className={`landing-contact-grid ${contactVisible ? 'animate-in' : ''}`}>
            ...
            <form className="landing-contact-form" onSubmit={handleSubmit}>
              ...
              <button type="submit" className="landing-btn-primary landing-btn-glow">
                <Send size={16} />
                Send Message
              </button>
            </form>
      ...
      <footer className="landing-footer">
        <div className="landing-container landing-footer-inner">
          <div className="landing-footer-brand">
            <div className="landing-logo">
              <img src="/assets/Rectangle.png" alt="4J Laundry" className="landing-logo-img" />
              <span><strong>4J</strong> Laundry Shop</span>
            </div>
            <p>Laundry service providers offer professionals laundering solutions to assist individuals and business in achieving optimal garment care and cleanliness</p>
          </div>
        ...
        <div className="landing-footer-bottom">
          <div className="landing-container">
            <p>Copyright &copy; {new Date().getFullYear()} 4j. All rights reserved</p>
          </div>
        </div>
      </footer>
    </div>
